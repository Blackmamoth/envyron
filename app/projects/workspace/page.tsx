...
import { ArrowLeft, Download, Copy, Search, ChevronDown, Edit2 } from "lucide-react"
...
const availableItems = [
	{ id: 1, name: "PostgreSQL Database", type: "service", description: "Relational database with connection pooling" },
	{ id: 2, name: "Redis Cache", type: "service", description: "In-memory data structure store" },
	{ id: 3, name: "Auth Service", type: "service", description: "JWT-based authentication system" },
	{ id: 4, name: "Email Service", type: "service", description: "SMTP email delivery service" },
	{ id: 5, name: "Next.js Starter", type: "template", description: "Full-stack Next.js application template" },
	{ id: 6, name: "API Gateway", type: "template", description: "Express.js API gateway with middleware" },
]

const mockEnvCode = `# Database Configuration
...
const mockTypeScriptCode = `import { z } from 'zod'
...
const mockZodCode = `import { z } from 'zod'
...
export default function ProjectWorkspace() {
	...
	const [activePreview, setActivePreview] = useState<".env" | "TypeScript" | "Zod">(".env")
	...
	const toggleItem = (id: number) => {
		setProjectItems((items) => items.map((item) => (item.id === id ? { ...item, enabled: !item.enabled } : item)))
	}
	...
	const downloadFile = () => {
		const content = getCurrentContent()
		const filename =
			activePreview === ".env" ? ".env.example" : activePreview === "TypeScript" ? "config.ts" : "env.schema.ts"
	...
	const getCurrentContent = (previewType?: string) => {
		...
		switch (preview) {
			case ".env":
				return generateEnvContent(enabledServices, isTypeSafe)
			case "TypeScript":
				return generateTypeScriptContent(enabledServices, isTypeSafe)
			case "Zod":
				return generateZodContent(enabledServices, isTypeSafe)
			default:
				return generateEnvContent(enabledServices, isTypeSafe)
		}
	}

	const generateEnvContent = (services: any[], typeSafe: boolean) => {
		let content = ""
	...
	const generateTypeScriptContent = (services: any[], typeSafe: boolean) => {
	...
	const generateZodContent = (services: any[], typeSafe: boolean) => {
		...
		<div className="min-h-screen bg-[#050A1C] text-white">
			...
			<header className="border-b border-gray-800 bg-[#0B1437] px-6 py-4">
				<div className="flex items-center justify-between">
					...
					<Button onClick={generateAndDownload} className="bg-[#006D77] hover:bg-[#83C5BE] text-white font-medium">
								...
								{projectItems.map((item) => (
									...
									>
										<div className="flex-1">
											<h3 className={`font-medium ${item.type === "service" ? "font-mono" : ""}`}>{item.name}</h3>
					...
					<div className="border-b border-gray-800 bg-[#0B1437]">
						<div className="flex items-center justify-between px-6 py-4">
							...
							<div className="flex items-center gap-2">
								...
								<Button size="sm" onClick={downloadFile} className="bg-[#006D77] hover:bg-[#83C5BE] text-white">
									<Download className="h-4 w-4 mr-2" />
									Download
								</Button>
							</div>
									...
									{getCurrentContent()
										...
										.map((line, index) => (
											<div key={index} className="flex">
												<span className="text-[#83C5BE] w-8 text-right mr-4 select-none">{index + 1}</span>
