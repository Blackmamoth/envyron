...
import { EnvVariableSchema, envVariableSchema, EnvVariableTypes } from "@/validation/service"
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query"
import { getVariablesQueryOptions, createVariableMutationOptions, updateVariableMutationOptions } from "@/lib/queryOptions/envVariable"
...
export default function ServiceBody({ serviceId }: Props) {
  ...
  const [initialVariableLength, setInitialVariableLength] = useState(0)

  const queryClient = useQueryClient()

  const createVariablesMutation = useMutation({
    ...createVariableMutationOptions(serviceId),
    onSuccess: () => {
  ...
  const updateVariablesMutation = useMutation({
    ...updateVariableMutationOptions(serviceId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["variables", serviceId] })
    },
    onError: (error) => {
  ...
  const { register, control, handleSubmit, reset, getValues, formState: { isValid, isDirty } } = useForm({
    resolver: zodResolver(envVariableSchema), defaultValues: {
      env_variables: []
    }
  })

  const { fields, append, update, remove } = useFieldArray({ name: "env_variables", control })

  const handleSync = (data: EnvVariableSchema) => {
    if (initialVariableLength === 0) {
      createVariablesMutation.mutate(data)
    } else {
      updateVariablesMutation.mutate(data)
    }
  }
  ...
  useEffect(() => {
    if (data?.variables) {
      ...
      setInitialVariableLength(data.variables.length)
    }
  }, [data, reset]);
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 pb-24 space-y-8">
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Button
              onClick={() => append({ key: '', type: 'STRING', required: false, defaultValue: '' })}
              variant="outline"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center h-10">
                        ...
                        />
                        <span className="ml-2 text-sm text-gray-300">{field.required ? "Required" : "Optional"}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Select value={field.type} onValueChange={(value) => {
                          const current = getValues(`env_variables.${index}`)
                          update(index, { ...current, type: value as EnvVariableTypes })
                        }}>
                          ...
                          <SelectContent className="bg-[#0B1437] border-gray-700">
                            ...
                            <SelectItem value="INT" className="text-white hover:bg-[#006D77]/20">
                              int
                            </SelectItem>
                            <SelectItem value="FLOAT" className="text-white hover:bg-[#006D77]/20">
                              float
                            </SelectItem>
